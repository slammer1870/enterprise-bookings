...
test.describe('User Booking Flow', () => {
  ...
  test.beforeAll(async ({ browser }) => {
    ...
    try {
      ...
      const hasClassOptions = await page.locator('table tbody tr, [data-testid*="list"] a').count() > 0
      ...
      // Select class option
      const classOptionCombobox = page.getByRole('combobox').first()
      ...
      // Set lock out time
      const lockOutInput = page.locator('input[type="number"]').last()
      await lockOutInput.fill('60')
      await page.waitForTimeout(500)
      ...
      // Look for home page
      const homePageLink = page.locator('a[href*="/pages/"][href*="home"], td:has-text("home"), td:has-text("Home")').first()
  ...
  test('should complete registration flow from homepage to magic link sent', async ({ page }) => {
    ...
    // Look for common patterns for lessons/classes on the homepage
    // This could be links, buttons, or cards with lesson information
    ...
    // If no lesson found via selectors, try to find any link with /bookings/ in href
    if (!lessonLink) {
      console.log('No lesson found via selectors, searching for /bookings/ links...')
      const allLinks = await page.locator('a[href*="/bookings/"]').all()
      if (allLinks.length > 0) {
        lessonLink = allLinks[0]
        lessonUrl = await lessonLink?.getAttribute('href') || ''
      }
    }
    
    // If still no lesson found, we need to create one or check the page structure
    if (!lessonLink) {
      console.log('No lessons found on homepage. Checking page content...')
      await page.screenshot({ path: 'test-results/screenshots/02-no-lessons-found.png', fullPage: true })
    ...
    // Click the lesson or navigate to it
    if (lessonLink && lessonUrl) {
    ...
    await page.waitForTimeout(2000)
    await page.screenshot({ path: 'test-results/screenshots/03-after-lesson-click.png', fullPage: true })
    ...
    await page.screenshot({ path: 'test-results/screenshots/04-complete-booking-page.png', fullPage: true })
    ...
    await page.screenshot({ path: 'test-results/screenshots/07-after-registration.png', fullPage: true })
    ...
    if (isOnMagicLinkPage) {
      ...
      await page.screenshot({ path: 'test-results/screenshots/08-magic-link-sent.png', fullPage: true })
    ...
    await page.screenshot({ path: 'test-results/screenshots/08-unexpected-page.png', fullPage: true })
  ...
  test('should show login form when accessing booking while logged out', async ({ page }) => {
    ...
    await page.screenshot({ path: 'test-results/screenshots/login-flow-complete.png', fullPage: true })
    
    // Test passes if we either redirected OR stayed on auth page (which is expected behavior)
    const isValidEndState = finalUrl.includes('/magic-link-sent') || 
