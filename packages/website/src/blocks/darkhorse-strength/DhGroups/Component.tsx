...
export const DhGroupsBlock: React.FC<GroupsBlockProps> = ({ heroImage, benefits, features, cta }) => {
      ...
      <div className="grid gap-4 md:grid-cols-2 md:gap-x-40 md:gap-y-12">
                ...
                <div key={index} className="flex items-center">
                  <div className="p-6">
                    {iconUrl ? (
                      <Image
                        src={iconUrl}
                        alt={getImageAlt(benefit.icon, benefit.text)}
                        width={40}
                        height={40}
        ...
        <div className="flex flex-col gap-4 md:col-span-2 md:flex-row md:items-center md:justify-between md:py-10">
          <div>
            ...
            <p className="mb-8 text-xl font-light text-yellow-600 md:text-2xl">
              {cta.description}
            </p>
          </div>
          <div className="w-full md:w-1/2">
            <FormBlock enableIntro={false} form={cta.form as never} />
          </div>
        </div>
      </div>
