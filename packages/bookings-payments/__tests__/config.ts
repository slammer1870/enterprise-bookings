...
/** Fresh config per call so integration suites never share mutated collections/plugins. */
export function createTestConfig(): Config {
  return {
    ...
    db: postgresAdapter({
      pool: {
        connectionString:
          process.env.DATABASE_URI || "postgres://postgres:postgres@localhost:5432/payload",
      },
    }),
