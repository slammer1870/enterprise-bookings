...
describe('class-pass-booking (Phase 4.6)', () => {
  ...
  it('returns only passes that match lesson tenant and allowed types', () => {
    ...
    const result = filterValidClassPassesForLesson(lesson, passes)
  ...
  it('returns empty when lesson has no allowedClassPasses', () => {
    const future = new Date(Date.now() + 86400000).toISOString()
    const lessonNoAllowed: LessonLike = {
      tenant: tenantId,
    ...
    const result = filterValidClassPassesForLesson(lessonNoAllowed, passes)
