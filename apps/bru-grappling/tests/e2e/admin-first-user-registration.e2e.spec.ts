...
test.describe('Admin First User Registration', () => {
  test('should redirect through login to create-first-user page when no users exist', async ({ page }) => {
    ...
    if (!canReach) {
      ...
      test.info().skip(true, 'Users already exist in database - this test requires a fresh database')
      return
    }
  ...
  test('should create first admin user successfully', async ({ page }) => {
    ...
    const roleCombobox = page.locator('text=Role').locator('..').locator('[role="combobox"]').first()
    ...
    const dashboardHeading = page.getByRole('heading', { name: /dashboard/i }).or(page.getByText(/dashboard/i)).first()
  ...
  test('should complete full flow from /admin to dashboard', async ({ page }) => {
    ...
    await expect(page.getByText('To begin, create your first user.')).toBeVisible({ timeout: 10000 })
    ...
    const dashboardContent = page.getByText(/dashboard/i).or(page.getByRole('heading', { name: /dashboard/i })).first()
