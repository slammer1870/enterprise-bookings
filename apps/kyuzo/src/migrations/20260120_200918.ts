...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  ...
  DO $$ BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = 'public' AND table_name = 'timeslots' AND column_name = 'date') THEN
      ALTER TABLE "timeslots" ALTER COLUMN "date" SET DEFAULT '2026-01-20T20:09:18.432Z';
    END IF;
  END $$;
...
export async function down({ db, payload, req }: MigrateDownArgs): Promise<void> {
  ...
  ALTER TABLE "timeslots" ALTER COLUMN "date" SET DEFAULT '2025-12-06T11:02:40.305Z';
