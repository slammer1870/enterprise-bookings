...
function getFlatStartTimeFilter(searchParams: LessonSearchParams): string | undefined {
  const direct =
...
export function getLessonStartTimeFilter(
  searchParams: LessonSearchParams | string | undefined,
): string | undefined {
  if (!searchParams) return undefined;

  if (typeof searchParams === "string") {
    return getLessonStartTimeFilter(
      qs.parse(searchParams, {
        ignoreQueryPrefix: true,
        depth: 6,
      }) as LessonSearchParams,
    );
  }
...
export function normalizeLessonSearchParams(
  searchParams: LessonSearchParams | string,
