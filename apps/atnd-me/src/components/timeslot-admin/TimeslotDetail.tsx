...
import { ManageTimeslot } from '@repo/bookings-plugin/src/components/lessons/manage-timeslot'
import { AddBooking } from '@repo/bookings-plugin/src/components/bookings/add-booking'
...
import { GroupedBookingRoster } from './GroupedBookingRoster'
...
export const TimeslotDetail = ({
  ...
  const [expandedGroups, setExpandedGroups] = useState<RosterBookerGroup[] | null>(null)
  ...
  const fetchTimeslotRoster = useCallback(
    async (signal?: AbortSignal) => {
      const res = await fetch(`/api/admin/timeslots/${timeslot.id}/roster`, {
        method: 'GET',
        credentials: 'include',
        signal,
      })
      if (!res.ok) {
        throw new Error(`Failed to fetch roster for timeslot ${timeslot.id}`)
      }
      const data = await res.json()
      const docs = (data?.docs ?? []) as Booking[]
      const groups = (data?.groups ?? []) as RosterBookerGroup[]
      const total = typeof data?.totalDocs === 'number' ? data.totalDocs : docs.length
      return { docs, groups, total }
    },
    [timeslot.id],
  )
  ...
  const refetchExpandedBookings = useCallback(async () => {
    ...
    try {
      const { docs, groups, total } = await fetchTimeslotRoster(controller.signal)
  ...
  }, [isExpanded, timeslot.id, expandedBookings, fetchTimeslotRoster, router])

  return (
    <>
      <TableRow
        key={timeslot.id}
        className={cn('[&_td]:py-1.5', !isActive && 'opacity-50 hover:opacity-70')}
      >
        ...
        <TableCell>
          <Button size="small" buttonStyle="secondary" onClick={() => toggleBookings(timeslot.id)}>
            {bookingCount}
          ...
          <TableCell colSpan={6}>
            <div className="rounded-md p-2">
              {isLoadingExpandedBookings ? (
                ...
                <GroupedBookingRoster
                  groups={expandedGroups ?? []}
                  bookings={(expandedBookings ?? []) as Booking[]}
                  onBookingUpdated={refetchExpandedBookings}
                />
              )}
              {!isLoadingExpandedBookings && (
                <AddBooking timeslotId={timeslot.id} onBookingCreated={refetchExpandedBookings} />
              )}
            </div>
          </TableCell>
        </TableRow>
