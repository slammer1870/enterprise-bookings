import { admin, magicLink as magicLinkPlugin, openAPI } from 'better-auth/plugins'
...
import { isTestMagicLinkStoreEnabled, saveTestMagicLink } from './test-magic-link-store'
...
export function createBetterAuthPlugins({
  ...
  magicLinkDisableSignUp?: boolean
  adminUserIds: string[]
  sendMagicLink: (args: { email: string; token: string; url: string }) => Promise<void>
  ...
  if (enableMagicLink) {
    plugins.push(
      magicLinkPlugin({
        ...
        ...(typeof magicLinkDisableSignUp === 'boolean' ? { disableSignUp: magicLinkDisableSignUp } : {}),
      }),
    )
  }
...
export function createBetterAuthOptions(config: BetterAuthServerConfig) {
  const baseURL = config.baseURL || process.env.NEXT_PUBLIC_SERVER_URL || 'http://localhost:3000'
  ...
  const options: any = {
    ...
    baseURL,
    secret:
      process.env.BETTER_AUTH_SECRET || process.env.PAYLOAD_SECRET || 'secret-fallback-for-development',
    trustedOrigins:
      config.trustedOrigins ||
      ([process.env.NEXT_PUBLIC_SERVER_URL || 'http://localhost:3000'].filter(Boolean) as string[]),
    emailAndPassword: {
      enabled: true,
      ...
      async sendResetPassword({ user, url }: { user: any; url: string }) {
        if (!enableOtherAuthEmails || !emailCfg.enabled) {
      ...
      deleteUser: {
        ...
        sendDeleteAccountVerification: async ({
          ...
          const html = buildBasicAuthEmailHtml({
            ...
            greeting: (_user as any)?.name ? `Hello, ${(_user as any).name}` : 'Hello',
