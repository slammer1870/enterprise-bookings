...
import { DeleteTimeslot } from "./delete-timeslot";
...
function getPublicBookingUrl(
  timeslotId: number,
  tenantSlugFromTimeslot: string | null | undefined
): string {
  if (typeof window === "undefined") return "";
  const tenantSlug =
    tenantSlugFromTimeslot ?? getTenantSlugFromCookie();
...
/**
 * Actions dropdown for a timeslot row. Uses a native button as trigger so Radix
 * can attach ref and open the menu (Payload Button does not forward ref).
 */
export const ManageTimeslot = ({
  timeslotId,
  tenantSlug,
}: {
  timeslotId: number;
  ...
  const handleCopyBookingLink = async () => {
    const url = getPublicBookingUrl(timeslotId, tenantSlug);
    try {
    ...
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <button
          ...
          aria-label="Open timeslot actions"
        >
      ...
      <DropdownMenuContent align="end" className="z-[100]">
        <DropdownMenuItem asChild>
          <Link
            href={`${collectionBasePath}/${timeslotId}`}
