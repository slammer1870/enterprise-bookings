...
test.describe('Drop-in promo code breakdown', () => {
  ...
  test('applied promo code updates the price breakdown and totals', async ({
    ...
    await expect(page.getByTestId('class-price-original')).toHaveText('€10.00')
    await expect(page.getByTestId('class-price')).toContainText('€8.00')
    await expect(page.getByTestId('promo-discount')).toHaveText('-€2.00')
    await expect(page.getByTestId('booking-fee')).toHaveText('€0.80')
    await expect(page.getByTestId('total-original')).toHaveText('€11.00')
    await expect(page.getByTestId('total')).toContainText('€8.80')
    await expect(page.getByTestId('payment-total-original')).toHaveText('€11.00')
