...
export function KioskTimeslotCard({ timeslot, users }: { timeslot: Timeslot; users: User[] }) {
  const trpc = useTRPC()
  ...
  const kioskCheckIn = useMutation(
    trpc.bookings.kioskCreateOrConfirmBooking.mutationOptions({
      onSuccess: async () => {
        // Query-level revalidation: refetch kiosk timeslots so bookings/capacity update.
        await queryClient.invalidateQueries({ queryKey: trpc.timeslots.getForKiosk.queryKey() })
      },
    }),
  )
  ...
  const handleCheckIn = async () => {
    if (!value) return
    try {
      await kioskCheckIn.mutateAsync({ timeslotId: timeslot.id, userId: Number(value) })
      setError(null)
      setOpen(false)
      setValue(null)
  ...
  // Fall back to computing it from eventType.places - confirmed bookings.
  // Also handle the case where eventType might be just an ID (not populated).
  const eventTypeObj =
    typeof timeslot.eventType === 'object' && timeslot.eventType !== null ? timeslot.eventType : null
  const places = typeof eventTypeObj?.places === 'number' ? eventTypeObj.places : 50 // Default to 50 if missing
  const confirmedBookings = Array.isArray(timeslot.bookings?.docs)
    ? timeslot.bookings.docs.filter((b) => b?.status === 'confirmed').length
    : 0
  const computedRemaining = Math.max(places - confirmedBookings, 0)
  const remainingCapacity =
    typeof (timeslot as any).remainingCapacity === 'number' ? (timeslot as any).remainingCapacity : computedRemaining
  ...
  return (
    <div>
      <Card
        ...
        data-testid={`kiosk-timeslot-card-${timeslot.id}`}
      >
        <Collapsible>
          <CardHeader>
            <div className="flex justify-between items-center">
              <div className="flex flex-col gap-2">
                ...
                <CardTitle className="flex justify-between items-center">
                  {eventTypeObj?.name ?? 'Class'} {timeslot.location && `- ${timeslot.location}`}
                </CardTitle>
              </div>
          ...
          <CollapsibleContent>
            <CardContent>
              ...
              <div className="flex flex-col gap-2 mt-2" data-testid="kiosk-bookings-list">
                {timeslot.bookings.docs.length === 0 && <p>No bookings</p>}
                {timeslot.bookings.docs.map(
                  (booking) =>
