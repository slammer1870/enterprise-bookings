...
/**
 * Helper to save a class option and wait for navigation to the edit page.
 */
async function saveClassOptionAndWaitForNavigation(page: any): Promise<void> {
  const saveButton = page.getByRole('button', { name: 'Save' })
  await saveButton.waitFor({ state: 'visible', timeout: 30000 })
  
  // Start waiting for navigation before clicking
  const navigationPromise = page.waitForURL(/\/admin\/collections\/class-options\/\d+/, { 
    timeout: process.env.CI ? 120000 : 60000 
  })
  const responsePromise = page.waitForResponse(
    (response: any) =>
      response.url().includes('/api/class-options') &&
  ...
  try {
    await Promise.race([
      ...
      responsePromise.then(() => {
        ...
        return page.waitForURL(/\/admin\/collections\/class-options\/\d+/, { 
  ...
  // Final URL check with longer timeout for CI
...
async function selectClassOptionAndSaveLesson(page: any, className: string): Promise<void> {
  ...
  // Start waiting for navigation before clicking
  const navigationTimeout = process.env.CI ? 120000 : 30000
  const navigationPromise = page.waitForURL(/\/admin\/collections\/lessons\/\d+/, { timeout: navigationTimeout })
  const responsePromise = page.waitForResponse(
    (response: any) =>
  ...
  try {
    await Promise.race([
      ...
      responsePromise.then(() => {
        // If we got a response but no navigation yet, wait a bit more
        return page.waitForURL(/\/admin\/collections\/lessons\/\d+/, { 
          timeout: process.env.CI ? 30000 : 15000 
        })
      }),
    ])
...
async function expectLessonVisibleForTomorrow(
  ...
  await page.goto('/admin/collections/lessons', { waitUntil: 'domcontentloaded', timeout: process.env.CI ? 180000 : 120000 })
  await expect(page).toHaveURL(/\/admin\/collections\/lessons/, { timeout: process.env.CI ? 30000 : 10000 })
  await page.waitForLoadState('load', { timeout: process.env.CI ? 30000 : 15000 }).catch(() => {})
  ...
  const calendarVisible = await calendarContainer.isVisible({ timeout: process.env.CI ? 30000 : 10000 }).catch(() => false)
  if (!calendarVisible) {
...
test.describe('Admin Lesson Creation Flow', () => {
  ...
  test('should create lesson for class option with drop-in payment only', async ({ page }) => {
    ...
    await saveClassOptionAndWaitForNavigation(page)
