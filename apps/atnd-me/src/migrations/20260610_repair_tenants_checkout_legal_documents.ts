...
/**
 * Repair: `20260608_tenants_checkout_legal` was rewritten after it had already run in
 * production (first version added `checkout_legal_*` columns on `tenants`; the updated
 * version creates `tenants_checkout_legal_documents`). Payload records migrations by
 * name only, so redeploys skip the revised `up` and the array table is never created.
 *
