...
export const createCheckoutSession: PayloadHandler = async (req): Promise<Response> => {
  ...
  try {
    ...
    return new Response(
      JSON.stringify({ client_secret: checkoutSession.client_secret, url: checkoutSession.url }),
      { status: 200 }
    );
  } catch (error) {
    console.log("ERROR", error);
    return new Response(
      JSON.stringify({ message: "There was an issue creating a checkout session", status: 500 }),
      { status: 500 }
    );
  }
};
