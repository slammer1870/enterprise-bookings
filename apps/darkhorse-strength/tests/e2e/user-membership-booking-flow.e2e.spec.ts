...
test.describe('Darkhorse Strength: membership booking flow', () => {
  ...
  test('user can register, subscribe, and booking is confirmed on dashboard', async ({ page }) => {
    ...
    const lessonId = await createTimeslotViaApi(page, {
      eventTypeId,
      date: tomorrow,
      startHour: 10,
      startMinute: 0,
    ...
    await page.goto(`/admin/collections/timeslots/${lessonId}`, {
      waitUntil: 'load',
    ...
    // Plan-required timeslots show "Book" or "Book Trial Class"; direct check-in shows "Check In"
    ...
    await mockSubscriptionCreatedWebhook(page.context().request, { timeslotId: lessonId, userEmail: email })
