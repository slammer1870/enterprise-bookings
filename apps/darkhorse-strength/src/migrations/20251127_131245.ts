...
export async function up({ db, payload: _payload, req: _req }: MigrateUpArgs): Promise<void> {
  ...
  DO $$ BEGIN
    ...
    IF EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'staffMembers_image_id_media_id_fk') THEN
      ALTER TABLE "staffMembers" DROP CONSTRAINT "staffMembers_image_id_media_id_fk";
    END IF;
    
    -- Rename image_id to profile_image_id if it exists
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'staffMembers' AND column_name = 'image_id')
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'staffMembers' AND column_name = 'profile_image_id')
  ...
  ALTER TABLE "timeslots" ALTER COLUMN "date" SET DEFAULT '2025-11-27T13:12:45.317Z';
  ...
  END $$;
  DO $$ BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'staffMembers_profile_image_id_media_id_fk')
      AND EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'staffMembers' AND column_name = 'profile_image_id') THEN
  ...
  DO $$ BEGIN
    -- Clean up invalid instructor_id references in timeslots
    -- Set instructor_id to NULL where it doesn't exist in staffMembers table
    UPDATE "timeslots"
