import type { TaskHandler } from 'payload'
import { createGenerateTimeslotsFromScheduleHandler } from '@repo/bookings-plugin'

import { ATND_ME_BOOKINGS_COLLECTION_SLUGS } from '@/constants/bookings-collection-slugs'

/** Must match bookingsPlugin slugs — the package default handler targets `timeslots`, not `timeslots`. */
const generateTimeslotsForAtndMe = createGenerateTimeslotsFromScheduleHandler(
  ATND_ME_BOOKINGS_COLLECTION_SLUGS,
)

/**
 * Wrapper for generateTimeslotsFromSchedule that ensures tenant context is set
 * This allows the job to find tenant-scoped timeslots correctly
 */
export const generateTimeslotsFromScheduleWithTenant: TaskHandler<'generateTimeslotsFromSchedule'> = async (args) => {
  const { input, req } = args

  // Extract tenant from input (passed from scheduler document)
  const tenantId = (input as { tenant?: number }).tenant

  // Set tenant context in req so multi-tenant plugin filters queries correctly
  // This must be set before calling the original handler
  if (tenantId) {
    // Ensure req.context exists
    if (!req.context) {
      req.context = {}
    }
    req.context.tenant = tenantId
  }

  return generateTimeslotsForAtndMe(args)
}
