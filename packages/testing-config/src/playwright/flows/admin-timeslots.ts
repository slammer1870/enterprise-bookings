...
async function pickTimeslotTime(
  page: any,
...
export async function createEventType(
  ...
  await p.goto('/admin/collections/event-types', {
...
export async function setTimeslotDateAndTime(
  page: Page,
  targetDate: Date,
  options?: {
    startLabels?: string[]
  ...
  const endTimeInput = await pickTimeslotTime(p, 'endTime', options?.endLabels ?? ['11:00 AM', '11:00'])
  await expect(endTimeInput).not.toHaveValue('', { timeout: 10000 })
}

/**
 * Set the timeslot date to tomorrow and time to 10:00–11:00.
 * Returns the Date instance for tomorrow.
 */
export async function setTimeslotTomorrowAtTenToEleven(page: Page): Promise<Date> {
  const tomorrow = new Date()
  tomorrow.setDate(tomorrow.getDate() + 1)

  await setTimeslotDateAndTime(page, tomorrow)
...
export async function selectEventTypeInTimeslotForm(page: Page, className: string): Promise<void> {
  ...
  await eventTypeCombobox.fill(className)
...
export async function saveTimeslot(page: Page): Promise<void> {
...
/**
 * Create a timeslot via the REST API (avoids flaky admin date/time pickers).
 * Returns the created timeslot ID.
 */
export async function createTimeslotViaApi(
  page: Page,
  opts: {
    eventTypeId: number
    date: Date
  ...
  if (!res.ok()) {
    ...
    throw new Error(`Failed to create timeslot via API: ${res.status()} ${txt}`)
  }

  const json: any = await res.json().catch(() => null)
  const id = json?.doc?.id ?? json?.id
  if (id == null) throw new Error(`Unexpected create timeslot response: ${JSON.stringify(json)}`)
  return Number(id)
}
