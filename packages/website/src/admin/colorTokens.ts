...
export const COLOR_OPTIONS: ColorPresetOption[] = []
...
for (const [category, shades] of Object.entries(TAILWIND_PALETTE)) {
  shades.forEach((hex, index) => {
    ...
    COLOR_OPTIONS.push({ token, hex, category })
  })
}
...
export const COLOR_PRESETS: ColorPresetOption[] = [
  ...
  .map((token) => COLOR_OPTIONS.find((option) => option.token === token))
  .filter((option): option is ColorPresetOption => Boolean(option))

const CSS_COLOR_PATTERN =
  /^(#[0-9a-fA-F]{3,8}|hsl\([^)]+\)|hsla\([^)]+\)|rgb\([^)]+\)|rgba\([^)]+\)|oklch\([^)]+\)|[a-zA-Z]+)$/

/** Resolve Tailwind token or passthrough CSS color to a CSS-ready value. */
export function resolveColorToken(value: string | null | undefined): string | null {
...
/** Normalize hex for color inputs (#rrggbb). */
