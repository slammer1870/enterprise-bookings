...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  try {
    ...
    // Step 3: Handle scheduler_week_days_time_slot table if it exists
    ...
    if (schedulerTableExists.rows?.[0]?.exists) {
      ...
      const schedulerUsersNeedingInstructors = await db.execute<{
        user_id: number;
        user_name: string | null;
      }>(sql`
        SELECT DISTINCT
        ...
        for (const row of schedulerUsersNeedingInstructors.rows) {
          try {
            ...
            if (!existingInstructor.rows || existingInstructor.rows.length === 0) {
              ...
              console.log(`Created instructor ${newInstructor.id} for user ${row.user_id} (from scheduler)`);
            }
          } catch (error) {
            console.error(`Error creating instructor for user ${row.user_id} (from scheduler):`, error);
          }
        }
      }

      const schedulerToUpdate = await db.execute<{
        slot_id: number;
        user_id: number;
        instructor_id: number;
      }>(sql`
        ...
        INNER JOIN users u ON s.instructor_id = u.id
        INNER JOIN instructors i ON i.user_id = u.id
        LEFT JOIN instructors i_check ON i_check.id = s.instructor_id
        WHERE s.instructor_id IS NOT NULL
          AND i_check.id IS NULL
      `);

      if (schedulerToUpdate.rows && schedulerToUpdate.rows.length > 0) {
        console.log(`Found ${schedulerToUpdate.rows.length} scheduler time slots with user references that need to be updated`);

        for (const row of schedulerToUpdate.rows) {
          try {
            await db.execute(sql`
              ...
              WHERE id = ${row.slot_id}
                AND instructor_id = ${row.user_id}
            `);
            console.log(`Updated scheduler slot ${row.slot_id}: user ${row.user_id} -> instructor ${row.instructor_id}`);
          } catch (error) {
            console.error(`Error updating scheduler slot ${row.slot_id}:`, error);
          }
        }
      } else {
