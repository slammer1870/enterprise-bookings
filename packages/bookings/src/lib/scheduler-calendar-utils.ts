import { startOfWeek, addDays, setHours, setMinutes, getDay } from "date-fns";
 ...
 */
const DAYS_OF_WEEK: readonly string[] = ["Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday", "Sunday"] as const;
...
/**
 * Get the day name from an index (0-6)
 */
export function getDayName(dayIndex: number): string {
  ...
  return DAYS_OF_WEEK[0] as string;
}
...
export function calendarEventsToScheduler(
    ...
    if (dayIndex >= 0 && dayIndex < 7) {
      ...
      if (day) {
        ...
        if (existingDay?.timeSlot) {
          const existingSlot = existingDay.timeSlot.find(
            (s: TimeSlotData) => s.id === event.id
          );
  ...
  // Ensure each day has a timeSlot array (never undefined)
  days.forEach((day) => {
    if (!day.timeSlot) {
      day.timeSlot = [];
    }
  ...
  while (days.length < 7) {
    days.push({ id: undefined, timeSlot: [] });
  }
  if (days.length > 7) {
    days.length = 7; // Trim if somehow more than 7
  }
 ...
 * @returns Calendar event for the new slot
 */
export function createNewTimeSlotEvent(
  start: Date,
  end: Date,
  dayIndex: number
): CalendarEvent {
 ...
 * @param location1 - Location of first slot (optional)
 * @param location2 - Location of second slot (optional)
 * @returns true if slots overlap (and locations match if provided)
 */
 ...
 * @param newSlot - The new or updated slot
 * @param existingSlots - Array of existing slots (excluding the slot being updated if editing)
 * @param excludeSlotId - Optional ID of slot to exclude from validation (when editing)
